...
public class ExerciceGroupingBy {
    ...
    private void groupBy1() {
        ...
        Map<Trader,List<Transaction>> transac =
                transactions.stream()
    ...
    private void groupBy2() {
        System.out.println("GroupBy2");
         ...
         Map<Trader,Long> list = transactions
                 .stream()
    ...
    private void groupBy3() {
        ...
        Map<TransactionsLevel,List<Transaction>> list=transactions
                .stream()
                .collect(Collectors.groupingBy(transaction -> {
                   if(transaction.getValue()>=1000){
                       return TransactionsLevel.VERY_HI;
                   } else if(transaction.getValue()>=800 && transaction.getValue()<1000){
                       return TransactionsLevel.HI;
                   }else if(transaction.getValue()>=600 && transaction.getValue()<800 ){
